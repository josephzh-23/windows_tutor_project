...
class PrivateChatRoom(models.Model):
    ...
    #Basically add the user to the list
...
class UnreadChatRoomMessages(models.Model):
    ...
    # This is what we need to update the verb
    ...
    # Generic relation like we had before
    notifications = GenericRelation(Notification)

    def __str__(self):
        return f"Messages that {str(self.user.username)} has not read yet."

    # Here overriding the save function here
    def save(self, *args, **kwargs):
        if not self.id:  # if just created, add a timestamp.
            # Otherwise do not automatically change it ever.
            self.reset_timestamp = datetime.now()

        # Using super we will override this particular function whenever
        # this model is saved.
...
# This fxn will run before an unread_chatroom_message is saved

# Ex: if 1 user in a chatroom, sb sends a msg,
# will update a unread chatroom msg obj, and create a notification
    ...
    if instance.id is None:  # new object will be created
        ...
        if previous.count < instance.count:  # if count is incremented
            ...
            try:
                ...
                # Here we are updating that notification
